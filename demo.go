...
// OrderItem struct representing an item in an order request
...
func MakeOrder(orderRequest OrderRequest) ([]Order, error) {
  ...
  for _, orderItem := range orderRequest.Items {
    // Check if the inventory item is available
    inventoryItem, err := GetInventoryItem(InventoryQuery{ID: orderItem.Item.ID})
    if err != nil {
      return nil, err
    }
